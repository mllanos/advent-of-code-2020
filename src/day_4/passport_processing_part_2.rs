...
pub fn run() {
    ...
    let keys = [
        ...
        ("hgt", Regex::new("^1([5-8][0-9]|9[0-3])cm|(59|6[0-9]|7[0-6])in$").unwrap()),
    ...
    for passport in passports.iter() {
        ...
        if keys.iter().all(|(key, re)| passport_hashmap.contains_key(key) && re.is_match(passport_hashmap.get(key).unwrap())) {
            matches = matches + 1;
        }
    }
